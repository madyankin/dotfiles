...
~/.config/
  ...
  yadm/
    ...
    alfred/                   Alfred preference bundle + workflows.txt
...
`.config/yadm/hooks/pre_commit` refuses any commit whose staged content contains
something shaped like a credential — `sk-…`, `ghp_…`, `AKIA…`, Slack tokens, a
private key header, an npm `_authToken`. The cron sync commits unattended every
two hours, so without it a secret in a tracked file would reach a public repo
with nobody looking.

Patterns are strict enough that prose describing a key format does not trip
them. To bypass once: `YADM_HOOK_SKIP=1 yadm commit …`.
...
- **Editing the README through the index clears its skip-worktree bit.** The
  README is tracked but excluded from the work tree, which git implements with
  skip-worktree. `git update-index --cacheinfo` rewrites the entry and drops
  that bit, after which git reports the file as deleted and every commit carries
  a phantom deletion. Run `yadm sparse-checkout reapply` after editing it.
...
Alfred's sync folder points at `~/.config/yadm/alfred`, so the preference bundle
is version-controlled directly rather than going through iCloud. Only part of it
is tracked:

| Part | Tracked | Why |
|---|---|---|
| `preferences/`, `themes/`, `remote/` | yes | settings, web searches, hotkeys, Alfred Remote pages |
| `preferences/local/` | no | Alfred keeps per-machine hotkeys and appearance here |
| workflows you wrote | yes | nothing else can restore them |
| third-party workflows | no | 17 MB of code and ~80 third-party binaries; reinstalled instead |
| any `workflows/*/prefs.plist` | **never** | this is where workflows keep settings, and at least one keeps a plaintext API key there |

A workflow counts as third-party when its `info.plist` has a `webaddress`.
`scripts/alfred.sh` uses that split:

```
alfred.sh save      record third-party workflows in alfred/workflows.txt
alfred.sh install   reinstall anything listed but missing
alfred.sh sync      install missing, then report installed-but-unlisted
alfred.sh link      point Alfred's sync folder here (Alfred must be quit)
```

`install` fetches the latest `.alfredworkflow` release for GitHub-hosted
workflows and opens it for Alfred to import; for anything else it opens the
project page, because there is nothing reliable to download. The sync script
runs `alfred.sh save` every two hours, so the list does not drift.

If you write a new workflow of your own, `save` will warn that it is untracked —
add a negation for its directory to `.gitignore`, the same way `App launcher`
has one.

The Powerpack licence (`~/Library/Application Support/Alfred/powerpack.*.dat`)
and the clipboard database live outside the bundle and are not tracked.
