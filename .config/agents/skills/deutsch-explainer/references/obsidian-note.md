...
появляется только если вольт настроен и Obsidian запущен.
...
Создание **новой** заметки explainer'а в `explainer_folder` — это и есть заказанная работа,
отдельного подтверждения не требует. Правка ключей `explainer_*` в `Deutsch.md` — требует.
