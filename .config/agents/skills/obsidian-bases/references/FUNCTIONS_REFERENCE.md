...
| `html()` | `html(string): html` | Render as HTML — ⚠ **always call `escapeHTML()` first on any note property value** (Obsidian runs in Electron; unsanitised XSS = code execution) |
| `escapeHTML()` | `escapeHTML(string): string` | Escape HTML characters — use before every `html()` call on untrusted data |
