...
Выбирай **1–4 фигуры**, которые этому материалу реально нужны; в режимах `text` и `grammatik`
минимум две (сборщик проверяет). `quiz` — всегда. `glossen` — обязательно в режиме `text`.
Лишняя фигура хуже, чем её отсутствие, а декоративный график — хуже лишней фигуры.
Фигура заменяет абзац: поставил схему — текст рядом сожми до одной строки.
...
const DATA = {
  ...
  leitbeispiel: { de: "...", ru: "...", ziel: "За чем следить", grenze: "Где подсказка перестаёт работать" },
  merksatz:  ["Двигается → Akkusativ. Стоит → Dativ."],   // 1–3 строки, ≤12 слов каждая
  ...
  analyse:   [{ de: "фрагмент", wort: "дословно", frei: "нормальный перевод", warum: "один абзац",
                ...
                details: [{ titel: "Уточнение", text: "Сворачиваемая глубина" }] }],
...
У каждой фигуры есть общие поля: безопасный уникальный `id`, `titel`, `aria` (самодостаточное
текстовое описание смысла схемы), `hinweis` («что покрутить»), `beobachtung`
(«что ты должен был заметить» — скрыто до первого взаимодействия). Каждый `id` встречается
ровно один раз как `analyse[].figure_id`, поэтому схема стоит рядом с нужным объяснением.
У каждого вопроса есть `fokus`; минимум один вопрос содержит `figure_id` существующей схемы.
